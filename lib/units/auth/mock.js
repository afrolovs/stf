...
module.exports = function (options) {
  ...
  app.get('/auth/mock/', function (req, res) {
    if (req.session.user) {
      // Если есть сессия - редирект с сохранением hash-части URL
      var redirectUrl = options.appUrl + (req.url.split('#')[1] || '')
    ...
    // Редирект с сохранением hash
    var redirectUrl = urlutil.addParams(options.appUrl, {
      jwt: token
    }) + (req.url.split('#')[1] || '')
